...
class MainAppViewModel() : ViewModel() {
    ...
    private val installStateUpdateListener = InstallStateUpdateListener { installState ->
        when (installState.installStatus) {
            InstallStatus.DOWNLOADED -> {
                // Отправляем событие в Compose для отображения Snackbar
            ...
            InstallStatus.DOWNLOADING -> {
                ...
                // Здесь можно добавить отображение прогресса (пока не требуется)
            }
    ...
    override fun onCleared() {
        super.onCleared()
        if (::ruStoreAppUpdateManager.isInitialized) {
            ruStoreAppUpdateManager.unregisterListener(installStateUpdateListener)
        }
    }

    fun init(context: Context) {
        // В реальном проекте лучше через DI, здесь - упрощённый вариант
                    ...
                    ruStoreAppUpdateManager
                        ...
                        .addOnSuccessListener { resultCode ->
                            if (resultCode == Activity.RESULT_CANCELED) {
                                ...
                                Log.i(TAG, "User don't want to update")
                            }
                        }
